...
export default function InvitePage() {
  ...
  if (error && !info) return <AuthFrame title="Invite unavailable" subtitle={error}><Button className="w-full" onClick={() => router.push("/login")}>Sign in</Button></AuthFrame>;
  ...
  return (
    <AuthFrame title={`Join ${info.workspace.name}`} subtitle={`You were invited as ${info.role} (${info.email}).`}>
      <form onSubmit={accept} className="space-y-4">
        ...
        {info.hasAccount && !info.signedIn && <p className="text-[13px] text-muted">You already have an account. <a className="text-accent underline" href="/login">Sign in</a> first, then open this link again.</p>}
